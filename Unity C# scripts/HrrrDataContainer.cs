...
public class HrrrDataContainer : MonoBehaviour
{
    ...
    public void LoadFromResource()
    {
        ...
        for (int i = 1; i < lines.Length; i++)
        {
            ...
            if (fU < uMin) uMin = fU; if (fU > uMax) uMax = fU;
            if (fV < vMin) vMin = fV; if (fV > vMax) vMax = fV;
            if (fW < wMin) wMin = fW; if (fW > wMax) wMax = fW;
            if (fMag < magMin) magMin = fMag; if (fMag > magMax) magMax = fMag;
        }
